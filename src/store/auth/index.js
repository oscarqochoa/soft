...
export default {
  ...
  getters: {
    ...
    userRole: state => {
      const userModuls = state.currentUser.arrRoles.map(role => role.module_id)
      if (!userModuls.includes(state.module)) return -1
      return state.currentUser.arrRoles.filter(role => role.module_id === state.module)[0].role_id
    },
  },
  mutations: {
    ...
    SET_CURRENT_USER_ROLE_ID(state) {
      state.currentUser.role_id = state.currentUser.arrRoles.find(rol => rol.module_id === state.currentUser.modul_id).role_id
    },
  },
  actions: {
