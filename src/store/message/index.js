...
export default {
    ...
    mutations: {
        ...
        SET_LAST_CHAT_CONTACT_DATE(state, payload) {
            state.S_USER_CONTACTS.find(
                (el) =>
                    el.coworker_id === payload
        ...
        SET_STATUS_TO_LAST_MESSAGE(state, payload) {
            console.log(state.S_USER_MESSAGES, 'state.S_USER_MESSAGES')

            let obj = state.S_USER_MESSAGES.chat.chat.find(
                (el) =>
                    el.index == payload.index)
    ...
    actions: {
        ...
        async A_SAVE_MESSAGE_REPLY({ commit }, body) {
            try {
                ...
                const response = await MessageService.saveMessageReply(formData, body.endpoint)
                if (body.type) {
                    commit('SET_STATUS_TO_LAST_MESSAGE', { index: body.index, status: true, error: false })
                    if (body.originalMessageIndex) {
                        commit('DELETE_MESSAGE_WITH_ERROR_INDEX', body.originalMessageIndex)
                    }
                }
                return response
            } catch (error) {
                if (body.type) {
                    commit('SET_STATUS_TO_LAST_MESSAGE', { index: body.index, status: false, error: true })
                    if (body.originalMessageIndex) {
                        commit('DELETE_MESSAGE_WITH_ERROR_INDEX', body.originalMessageIndex)
                    }
                }
