    ...
    >
      ...
      >
        ...
        <template #cell(type)="data">
          <b-row class="cursor-pointer text-important" @click="openModalProgram(data.item)">
            <b-col v-if="data.item.type ===1" v-b-tooltip.bottom="'Add'">AD</b-col>
            <b-col v-else v-b-tooltip.bottom="'Change'">CH</b-col>
          </b-row>
        </template>
        <template #cell(first_program)="data">
          <span v-if="data.item.first_program === 'Business'" v-b-tooltip.bottom="'Business'">BU</span>
          <span
          ...
          >GS</span>
          <span v-if="data.item.first_program === 'Specialist'" v-b-tooltip.bottom="'Specialist'">SP</span>
          <span v-if="data.item.first_program === 'KeyBook'" v-b-tooltip.bottom="'KeyBook'">KB</span>
          <span v-if="data.item.first_program === 'Paragon'" v-b-tooltip.bottom="'Paragon'">PR</span>
        </template>
        <template #cell(program)="data">
          ...
          <span
            ...
            v-b-tooltip.bottom="'Credit Experts'"
          >CE</span>
          <span v-if="data.item.program === 'Debt Solution'" v-b-tooltip.bottom="'Debt Solution'">DS</span>
        ...
        <template v-slot:cell(initial_amount)="data">
          <div class="cursor-pointer" @click="openInitialPaymentModal(data.item)">
        ...
        <template v-slot:cell(contract_fee_status)="data">
          <b-icon
            :class="data.item.initial_payment_status != 1 ? 'cursor-pointer' : ''"
            icon="file-text"
            :variant="
        ...
        <template v-slot:cell(status)="data">
          <p
            ...
            :class="'color: text-' + status[data.item.status == 6 ? 1 : data.item.status].variant"
          >{{ status[data.item.status == 6 ? 1 : data.item.status].label }}</p>
        </template>
            ...
            <b-row v-if="data.item.creates > '2021-05-16 00:00:00'">
              ...
              <b-button
                v-if=" (data.item.status == 1 || data.item.status == 6) &&
                  G_IS_CEO && data.item.contract_fee_status == 1 &&
