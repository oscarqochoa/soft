...
export default {
  ...
  computed: {
    ...
    generatedUrl() {
      ...
      const encryptedSaleId = CryptoJS.AES.encrypt(
        ...
        .replaceAll('/', '-')
      // this.chargeUrl
      const californiaDate = moment().tz('America/Los_Angeles').format('YYYY-MM-DD')
  ...
  methods: {
    async changeCharge(checked) {
      const message = (checked) ? 'Are you sure to activate the charge?' : 'Are you sure to desactivate the charge?'
