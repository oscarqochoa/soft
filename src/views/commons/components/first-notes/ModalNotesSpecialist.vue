  ...
  methods: {
    ...
    answersNote() {
      return [
        ...
        { number: 1036, value: this.note.idTypeNumber.value ? this.note.idTypeNumber.value : 0 },
        { number: 1037, value: this.note.studentLoanType.value ? this.note.studentLoanType.value : 0 },
        { number: 1038, value: this.note.amountOwed.value ? this.note.amountOwed.value.toFixed(2) : 0 },
      ]
    },
    hideModal(status) {
      this.modalUp = false
    ...
    getDetailsAnswers(note) {
      note.forEach(answer => {
        if (answer.answer != 'null') {
          ...
          if (answer.question_id === 1038) this.note.amountOwed.value = parseInt(answer.answer, 10)
