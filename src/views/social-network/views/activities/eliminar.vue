...
export default {
  ...
  methods: {
    ...
    dataByDay(data) {
      ...
      array.forEach(element => {
        const letter = element.dayheader.toUpperCase()
        this.fields.push({
          key: 'field',
    ...
    async exportExcel() {
      ...
      const namefile = `schedules_report_${
  ...
  computed: {
    filterUser() {
      return this.items.filter(item => item.user_name
        .toLowerCase()
